...
void MyMap::readMapParkhaus(sensor_msgs::PointCloud2 &map_pc, MyPointCloud2D &map_carpark)
{
    ...
    try
    {
        io::CSVReader<2> in2("../catkin_ws/src/icp_lokalisierung/scan_matching/map/parkhaus.csv"); // Hier noch anpassen für Versuchsträger
        in2.read_header(io::ignore_extra_column, "x", "y");
        int i = 0;
        while (in2.read_row(x, y))
        {
    ...
    {
        io::CSVReader<2> in2("src/icp_lokalisierung/scan_matching/map/parkhaus.csv"); // Hier noch anpassen für Versuchsträger
        in2.read_header(io::ignore_extra_column, "x", "y");
        int i = 0;
        while (in2.read_row(x, y))
        {
